 ...
 */
public class DModelMulThread extends TreeModel{
    ...
    public static    int DEFAULT_THREADCOUNT=4;
    ...
    public DModelMulThread(int shuffle, int iter, int trees, double threshold, double rate, double weight) {
...
/*
    多线程操作，参数设置
 */
    public List<Thread> myThread=new ArrayList<>();
    public final CountDownLatch countDownLatch = new CountDownLatch(DModelMulThread.DEFAULT_THREADCOUNT);
    ...
    public void fit(List<String> documents, List<Integer> labels) throws Exception {
        ...
        System.out.println("线程数是:"+DModelMulThread.DEFAULT_THREADCOUNT);
        int interval = documents.size()/DModelMulThread.DEFAULT_THREADCOUNT;
        System.out.println("间隔是："+interval);

        for(int index=0; index < DModelMulThread.DEFAULT_THREADCOUNT;index++){
            List<String> tempList=documents.subList(index*interval,(index+1)*interval);
            List<Integer>tempLabels = labels.subList(index*interval,(index+1)*interval);
            Thread test1 = new Thread(  new Runnable(){
                @Override
                public void run() {
                    train(tempList,tempLabels);
                }
            });
            myThread.add(test1);
        }
        for(int index = 0; index <DModelMulThread.DEFAULT_THREADCOUNT;index++){
            myThread.get(index).start();
        }
